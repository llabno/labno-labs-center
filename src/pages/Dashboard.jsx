...
// Tier classification — maps project name patterns to bottleneck tiers
const TIER_KEYWORDS = [
...
const inferTier = (name) => {
  ...
  return null; // untiered projects
};

const TIER_ORDER = [1, 2, 3, null]; // null = untiered, shown last
const TIER_META = {
  1: { label: 'Tier 1 — This Week', color: '#d14040', bg: 'rgba(209,64,64,0.06)' },
  2: { label: 'Tier 2 — This Month', color: '#c49a40', bg: 'rgba(196,154,64,0.06)' },
  3: { label: 'Tier 3 — This Quarter', color: '#5a8abf', bg: 'rgba(90,138,191,0.06)' },
  null: { label: 'Other Projects', color: '#8a8682', bg: 'transparent' },
};
...
const Dashboard = () => {
  ...
  const [viewMode, setViewMode] = useState('tier'); // 'tier' or 'flat'
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <div style={{ display: 'flex', borderRadius: '8px', overflow: 'hidden', border: '1px solid rgba(0,0,0,0.08)' }}>
                <button onClick={() => setViewMode('tier')} style={{ padding: '6px 12px', fontSize: '0.75rem', fontWeight: 500, border: 'none', cursor: 'pointer', background: viewMode === 'tier' ? '#b06050' : 'rgba(255,255,255,0.5)', color: viewMode === 'tier' ? '#fff' : '#6b6764', transition: 'all 0.2s' }}>
                  Tier View
                </button>
                <button onClick={() => setViewMode('flat')} style={{ padding: '6px 12px', fontSize: '0.75rem', fontWeight: 500, border: 'none', cursor: 'pointer', background: viewMode === 'flat' ? '#b06050' : 'rgba(255,255,255,0.5)', color: viewMode === 'flat' ? '#fff' : '#6b6764', transition: 'all 0.2s' }}>
                  Flat View
                </button>
              </div>
          ...
          {viewMode === 'tier' ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {TIER_ORDER.map(tierKey => {
                ...
                const tierProjects = filteredProjects.filter(p => {
                  const t = inferTier(p.name);
                  return tierKey === null ? t === null : t?.tier === tierKey;
                });
                        ...
                        <tbody>
                          {tierProjects.map((proj) => {
                            ...
                            return (
                              <tr
                                key={proj.id}
                                style={{ borderBottom: '1px solid rgba(0,0,0,0.03)', background: isActive ? 'rgba(176,96,80,0.08)' : 'transparent', cursor: 'pointer', transition: 'background 0.25s ease' }}
                                onClick={() => toggleProjectBoard(proj)}
                              >
                                <td style={{ padding: '0.85rem 1rem', color: '#1e1d1c', fontWeight: 500 }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    <LayoutList size={16} color={isActive ? '#b06050' : '#8a8682'} /> {proj.name}
                                  </div>
                                </td>
                                <td style={{ padding: '0.85rem 1rem', width: '120px' }}>
                                  <span style={{ fontSize: '0.72rem', fontWeight: 600, padding: '3px 8px', borderRadius: '6px', background: urgency.bg, color: urgency.color, whiteSpace: 'nowrap' }}>
                                    {urgency.label}
                                  </span>
                                </td>
                                <td style={{ padding: '0.85rem 1rem', color: '#6b6764', fontSize: '0.85rem', width: '100px' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Calendar size={13}/> {formatDate(proj.due_date)}</div>
                                </td>
                                <td style={{ padding: '0.85rem 1rem', width: '80px' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <div style={{ flex: 1, height: '5px', background: 'rgba(0,0,0,0.06)', borderRadius: '4px', overflow: 'hidden', minWidth: '40px' }}>
              ...
              return (
                <div key={item.id} onClick={() => toggleProjectBoard(item)} style={{ padding: '1rem', background: isActive ? 'rgba(176,96,80,0.08)' : 'rgba(255,255,255,0.5)', border: '1px solid rgba(0,0,0,0.05)', borderRadius: '8px', cursor: 'pointer', borderLeft: `3px solid ${tier ? tier.color : '#b04a3a'}`, transition: 'all 0.25s ease' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '0.68rem', fontWeight: 600, padding: '2px 7px', borderRadius: '5px', background: urgency.bg, color: urgency.color, whiteSpace: 'nowrap', flexShrink: 0 }}>
                      {urgency.label}
                    </span>
                  </div>
